...
{
public:
	CExperimentsHandler(std::string &strMapLocationPath, std::string strTestSuiteLocationPath, double heurFactor = 1.0) 
		: m_MapManager(strMapLocationPath, strTestSuiteLocationPath)
	{
		...
		m_HPAStar.SetHeurFactor(heurFactor);
		m_JPSearch.SetHeurFactor(heurFactor);
		m_Fringe.SetHeurFactor(heurFactor);
		//m_Dijkstra.SearchPathTest();
		//int iNumExperiments = 100;
		//DoDijkstraExperiments(iNumExperiments);

		m_strTestSuiteLocationPath = strTestSuiteLocationPath;
...
private:
	...
	std::string m_strTestSuiteLocationPath;
