...
void CExperimentsHandler::DoTestSuiteExperiments()
{
	...
	std::string strTestMapSuite = std::regex_replace(m_strTestSuiteLocationPath, std::regex(".map"), ".txt");
	...
	if (testFileSuite.is_open())
	{
		...
		for (int i = 0; i < iTestCount; ++i)
		{
			...
			//iStartNode = 512 * 2 / 3 * 512 + 512 / 6;
			//iGoalNode = 512 / 2 * 512 + 512 * 5 / 6;
		   ...
		   // m_StatsAStar.AddStats(stats);
			...
			auto stats = m_HPAStar.GetStatistics();
			m_StatsHPAStar.AddStatsAvgHPAOpenSet(stats);
			...
			//auto stats = m_Fringe.GetStatistics();
