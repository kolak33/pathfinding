...
void CMapManager::DrawMap()
{
	...
	for(int i=0; i<m_Graph.size(); ++i)
	{
		if(m_Graph[i].GetGridTileInfo().GetGridTypeEnum() == GridTypePassable)
			glColor3f(0.9f, 0.9f, 0.9f); //WHITE-GREY
			//glColor3f(0.5f, 0.2f, 0.8f); //PURPLE
		else
			glColor3f(0.00f, 0.00f, 0.00f); //BLACK
...
void CMapManager::ColorSelectedTiles(std::vector<int> &vec, CColor *color)
{
	DrawShortestPath(vec, color);
}

void CMapManager::DrawShortestPath(std::vector<int> &vec, CColor *color)
{
	glBegin(GL_QUADS);
